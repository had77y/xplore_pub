...
#   /rover/cmd_vel (geometry_msgs/Twist) → publié à 10 Hz en mode race seulement
#
# MÉCANISME HOLD-TO-MOVE :
#   Les axes sont mis à zéro si aucune touche n'est pressée depuis AXIS_TIMEOUT.
#   Key repeat OS ≈ 30ms — AXIS_TIMEOUT 200ms : le rover s'arrête 200ms après relâchement.
...
class ControllerNode(Node):
    ...
    def stop_motors(self):
        with self.lock:
            ...
            self.last_linear_time = self.last_angular_time = 0.0
...
def print_arm():
    ...
    print('═' * 44)
    print('   (contrôles bras : utiliser rover_gui)')
    print('   M   →  Retour menu principal')
    print('═' * 44 + '\n')
...
def get_key(settings, timeout=0.05):
    # mode raw terminal : chaque touche lue sans attendre Entrée
    ...
    try:
        while rclpy.ok():
            ...
            if state == STATE_MENU_MAIN:
                ...
                elif key.lower() == 'q': node.set_axes(linear= 1.0, angular= 1.0)
                elif key.lower() == 'e': node.set_axes(linear= 1.0, angular=-1.0)
                elif key.lower() == 'y': node.set_axes(linear=-1.0, angular= 1.0)
                elif key.lower() == 'x': node.set_axes(linear=-1.0, angular=-1.0)
